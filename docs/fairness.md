...
It is not a good fit when absolute throughput isolation is required (dedicated queues per tenant remain or [task queue priorities](priority-task-queues.md) are the appropriate choice).
...
- **[Priority Task Queues](priority-task-queues.md)**: Order tasks by urgency level within the same Task Queue using `PriorityKey`.
- **[Downstream Rate Limiting](downstream-rate-limiting.md)**: Cap absolute throughput to a downstream service with a queue RPS setting.
